...
const OrderHistoryPage = () => {
  ...
  useEffect(() => {
    // localStorage에서 주문 내역 가져오기 (나중에 Supabase로 변경)
    const loadOrders = () => {
      const savedOrders = localStorage.getItem('ajouorder_history');
      if (savedOrders) {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'preparing':
        return <span className="px-2 py-1 bg-amber-100 text-amber-700 text-xs font-medium rounded-full">준비중</span>;
      case 'cancelled':
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              ...
              >
                ...
                {/* 카페명 */}
                <h3 className="font-bold text-gray-900 mb-2">{order.cafeName}</h3>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  ...
                  <button className="text-sm text-gray-500 hover:text-ajou-primary">
                    상세보기 →
                  </button>
                </div>
              </div>
